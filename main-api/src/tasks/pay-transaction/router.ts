...
router.get('/all',
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const payTransactions = await payTransactionService.getAllFromOrganizationByPeriod(organizationId, userInfo)
...
router.get('/',
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const employeeId = employee
            const queryLowered = q.toString().toLowerCase()
            const payTransactions = await payTransactionService.getAllFromOrganizationByEmployeeByPeriod(organizationId, employeeId, userInfo)
            const renamedPayTransactions = payTransactions.map(({
                id,
                employee_id,
                transaction_id,
