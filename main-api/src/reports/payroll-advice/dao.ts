...
const getAllFromOrganization = async(organizationId: string, branchId: string, departmentId: string): Promise<any[]> => {
...
const calculateGrossTaxable = (transactions: any) => {
    ...
    transactions.map((tran: any) => {
        if(tran.transaction_type_name === 'Earning Amount') {
            ...
            totalEarnings += parseFloat(tran.transaction_amount)
        }
    ...
    return {grossTaxable, totalEarnings, totalDeductions}
}
